...
class ConnectivityRepository(context: Context) {
    ...
    fun observeNetworkState(): Flow<NetworkStateUiModel> = callbackFlow {
    ...
    private fun buildState(manager: ConnectivityManager): NetworkStateUiModel {
        ...
        val networkId = if (Build.VERSION.SDK_INT >= 29) {
    ...
    private fun getNetworkCapabilityInt(name: String): Int? = runCatching {
        NetworkCapabilities::class.java.getField(name).getInt(null)
    }.getOrNull()

    private fun getNetworkTransportInt(name: String): Int? = runCatching {
        NetworkCapabilities::class.java.getField(name).getInt(null)
    }.getOrNull()
}
